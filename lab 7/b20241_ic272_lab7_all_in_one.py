...
plt.plot([1, 2,3,4], sorted(eigen_values,reverse=True), marker='o', markerfacecolor='blue')
...
for i in range(k):
    label = 'class ' + str(i)
    plt.scatter(combined.loc[combined['class'] == i, 'Column1'], combined.loc[combined['class'] == i, 'Column2'],
...
# calculate the distortion for each final cluster chosen by KMeans above.(used inbuilt here)
distortion = kmeans.inertia_

print("The Distortion measure for k=3 is: ", round(distortion, 3))
...
print("The purity score for k=3 is: ", round(find_purity(kmeans_prediction, df['Species']),3))
...
for k in arr:
    ...
    print("The Distortion measure and Purity score for k= " + str(k) + " is: ", round(distortion, 3), " and ",
          round(find_purity(kmeans_prediction, df['Species']) , 3))

    # saving value of distortion in J=[]
    J.append(find_distortion(k, combined, centres))
    # saving purity score in purity_score=[]
    purity_scores.append(round(find_purity(kmeans_prediction, df['Species']), 3))
...
for i in range(K):
    ...
    plt.scatter(combined.loc[combined['class'] == i, 'Column1'], combined.loc[combined['class'] == i, 'Column2'],
                label=label, alpha=0.85)
    # plotting cluster centre
    plt.scatter(centres_gmm[i][0], centres_gmm[i][1], color='black', marker="X", label="Centroids" if i == 0 else "")
...
print("The Purity score for GMM clustering for k=3 is: ", round(find_purity(gmm.predict(df2), df['Species']) , 3))
...
for k in arr:
    ...
    # saving purity score in purity_score=[]
    purity_scores.append(round(find_purity(gmm.predict(df2), df['Species']), 3) )
...
# defining function to assign test samples to clusters
...
for c in possible:
    ...
    print("The Purity score for epsilon=" + str(eps) + " and min_samples= " + str(min_samples) + " is: ",
          round(find_purity(dbtest, df['Species']) , 3))
